...
namespace EMullen.Core {
    ...
    public class PlayerManager : MonoBehaviour
    {
        ...
        public static PlayerDataRegistry DataReg => Instance.DataRegistry;
...
#region Runtime fields
        ...
        public PlayerDataRegistry DataRegistry { get; private set; }
...
#region Add/Remove player
        public void AddPlayer(PlayerInput input) 
        {
            ...
            string identifier = data.GetData<IdentifierData>().uid;
            if(DataRegistry.IsPlayerRegistered(identifier)) {
                Debug.LogError("Failed to add new player: Player is already registered");
                return;
            }

            DataRegistry.RegisterPlayer(data);
...
#region Input prompts
        ...
        public void UpdateMissingDevicesPrompt() 
        {
            ...
            if(playersMissingDevices.Count > 0) {
                playersMissingDevices.ForEach(playerInput => nameList += $"{DataRegistry.GetPlayerData(LocalPlayers[playerInput.playerIndex].UID)}, ");
                nameList = nameList[..^2];
            }
